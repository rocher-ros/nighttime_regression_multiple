...
night_fun <- function(site, reg_start, reg_end){
  ...
  daily = site %>% group_by(Daily = format(solar.time, "%Y-%m-%d")) %>%
  ...
  for(i in 2:length(daily$Daily)-1){
    ...
    day1 <- as.Date(daily$solar.time_mean[i])
    day2 <- as.Date(daily$solar.time_mean[i+1])
    hour1 <- '15:00:00'
    ...
    test<-subset(site, solar.time> Date1 & solar.time < Date2)
    ...
    for( j in 1:length(twindow)){
        ...
        nightr_d <- subset(test, solar.time> reg_per1 & solar.time < reg_per2)
    ...
    daily$slope.se[i] <- varyreg$slope.se[bestie]
    daily$r2[i]    <- varyreg2$r2[bestie]
    #finally convert the slope to k600 with the temperature correction
    daily$k600[i]<- ((600/(1800.6-(120.1*daily$temp.water_mean[i])+(3.7818*daily$temp.water_mean[i]^2)-(0.047608*daily$temp.water_mean[i]^3)))^-0.5)*varyreg$slope[bestie]
    ...
    ####It is disconnected by default, if you want to use it make sure you have light data also
          ...
          #   geom_line(aes(x=as.POSIXct(solar.time), y=DO.sat-DO.obs), size=2, color="blue3")+
          ...
          #   geom_line(aes(x=as.POSIXct(solar.time), y=deltaO2), size=2, color="darkred")+
          #   scale_x_datetime(date_breaks = "6 hour", labels=date_format("%d-%H"))+
          #   geom_vline(xintercept = as.numeric(reg_per1), linetype=4)+
          #   geom_vline(xintercept = as.numeric(reg_per2), linetype=4)+
          ...
          #   geom_line(aes(x=as.POSIXct(solar.time), y=light), size=2, color="goldenrod2")+
          #   scale_x_datetime(date_breaks = "6 hour", labels=date_format("%d-%H"))+
